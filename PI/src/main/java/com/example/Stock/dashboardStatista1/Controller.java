...
public class Controller {
    ...
    @FXML private BarChart<String, Number> quantityChart;
    @FXML private PieChart categoryChart;
    @FXML private AreaChart<Number, Number> stockEvolutionChart;
            ...
            for (int i = 0; i < alertes.length(); i++) {
                ...
                data.nodeProperty().addListener((obs, old, newNode) -> {
                    if (newNode != null) {
                        ...
                        Tooltip tooltip = new Tooltip(String.format("Produit: %s\nEntrepôt: %s\nJours avant rupture: %.2f\nQuantité: %.2f",
